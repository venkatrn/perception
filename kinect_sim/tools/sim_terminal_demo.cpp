...
main (int argc, char** argv)
{
  ...
  if (mode==0){
    ...
    m = AngleAxisd(-9.149, Vector3d::UnitZ())     * AngleAxisd(0, Vector3d::UnitY())    * AngleAxisd(0, Vector3d::UnitX());  
    pose *= m;
    Vector3d v;
    //v << 1.31762, 0.382931, 1.89533;
    v << 3.31762, 0.082931, 1.89533;
    pose.translation() = v;  
    for (int i=0;i< 10;i++){ // duplicate the pose 100 times
      poses.push_back(pose); 
    }
  }else if(mode==1){
