...
bins = {[0.01, 3*kDegree], [0.01 5*kDegree], [0.01 20*kDegree], [0.01 180*kDegree],
        [0.05, 3*kDegree], [0.05 5*kDegree], [0.05 20*kDegree], [0.05 180*kDegree],
        [0.1, 3*kDegree], [0.1 5*kDegree], [0.1 20*kDegree], [0.1 180*kDegree],
        [0.2, 3*kDegree], [0.2 5*kDegree], [0.2 20*kDegree], [0.2
        180*kDegree]};
...
map = containers.Map;
if (strcmp(method,'ourcvfh'))
  map = ourcvfh_map;
elseif (strcmp(method,'perch'))
  map = perch_map;
...
for key_cell = gt_map.keys
  ...
  rot_error = min(poses(:, end) - gt_poses(:, end),...
                  2 * pi - (poses(:, end) - gt_poses(:, end)));
