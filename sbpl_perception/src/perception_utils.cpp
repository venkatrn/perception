...
bool perception_utils::GetRectanglePoints(PointCloudPtr cloud, PointCloudPtr rectangle_points, vector<Eigen::Vector3f>* axes)
{
  ...
  else if (fabs(axis.dot((*axes)[0])) < 0.1 && axes->size() < 2) // get the orthogonal axes //0.2
  {
...
void perception_utils::GetRangeImageFromCloud(PointCloudPtr cloud, pcl::visualization::PCLVisualizer& viewer, pcl::RangeImagePlanar* range_image)
{
...
/*
pcl::RangeImagePlanar perception_utils::GetRangeImageFromCloud(PointCloudPtr cloud, pcl::visualization::PCLVisualizer& viewer)
{
  // Image size. Both Kinect and Xtion work at 640x480.
  int imageSizeX = 640;
  int imageSizeY = 480;
