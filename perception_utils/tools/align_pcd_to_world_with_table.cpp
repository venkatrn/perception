...
void GetDepthImageFromPointCloud(PointCloudPtr cloud,
  ...
  *original_cloud = *cloud;
  ...
  transformPointCloud(*original_cloud, *original_cloud, cam_to_body);
  ...
  demeanPointCloud(*original_cloud, centroid, *original_cloud);
  PointT center_point;
  ...
  transformPointCloud(*original_cloud, *original_cloud, transform);
  ...
  for (int ii = 0; ii < height; ++ii) {
    for (int jj = 0; jj < width; ++jj) {
      PointT p = original_cloud->at(jj, ii);

      if (isnan(p.z) || isinf(p.z)) {
        (*depth_image)[ii * width + jj] = 20000;
        original_cloud->at(jj, ii).r = 0;
        original_cloud->at(jj, ii).g = 0;
        original_cloud->at(jj, ii).b = 0;
      } else {
        (*depth_image)[ii * width + jj] = static_cast<unsigned short>(p.z * 1000.0);
      }
    }
